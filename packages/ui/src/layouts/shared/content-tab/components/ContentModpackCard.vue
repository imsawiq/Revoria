...
const observer = new ResizeObserver((entries) => {
	for (const entry of entries) {
		isExpanded.value = entry.contentRect.width >= 560
	}
})
onMounted(() => {
...
<template>
	<div
		ref="containerRef"
		class="flex flex-col gap-3 rounded-2xl border border-solid border-divider bg-bg-raised p-4"
		...
		<div class="flex flex-wrap items-start justify-between gap-4">
			...
			<div class="flex shrink-0 items-center gap-2">
				...
				<template v-else>
					...
					<!-- Collapsed actions -->
					<div v-if="!isExpanded && hasUpdate && hasUpdateListener" class="flex">
					...
					<Tooltip
						v-if="!isExpanded && collapsedOptions.length"
						theme="dismissable-prompt"
						:triggers="[]"
						:shown="showContentHint && !isExpanded"
						:auto-hide="false"
					...
					<ButtonStyled v-if="isExpanded && overflowOptions?.length" circular type="transparent">
