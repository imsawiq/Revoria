...
const messages = defineMessages({
	...
	filterUpdates: {
		id: 'content.page-layout.filter.updates',
		defaultMessage: 'Updates',
	},
	filterClientOnly: {
		id: 'content.page-layout.filter.client-only',
		defaultMessage: 'Client-only',
	},
	filterDisabled: {
		id: 'content.page-layout.filter.disabled',
...
const formatContentType = (label: string, count: number) => {
	const key = label as keyof typeof commonProjectTypeSentenceMessages
	const descriptor = commonProjectTypeSentenceMessages[key]
	if (descriptor) {
		return formatMessage(descriptor, { count })
	}
	return `${label}${count === 1 ? '' : 's'}`
}

const formatProjectTypeLabel = (label: string) => {
	const key = label as keyof typeof commonProjectTypeCategoryMessages
	const descriptor = commonProjectTypeCategoryMessages[key]
	if (descriptor) {
...
const { selectedFilters, filterOptions, toggleFilter, applyFilters } = useContentFilters(
	ctx.items,
	{
		...
		formatProjectType: formatProjectTypeLabel,
		updateFilterLabel: formatMessage(messages.filterUpdates),
		clientOnlyFilterLabel: formatMessage(messages.filterClientOnly),
		disabledFilterLabel: formatMessage(messages.filterDisabled),
	},
)
				...
				<div class="flex flex-col gap-4">
						...
						<StyledInput
							...
							:placeholder="
								formatMessage(messages.searchPlaceholder, {
									...
									contentType: formatContentType(ctx.contentTypeLabel.value, tableItems.length),
								})
							"
						/>
					...
					<div class="@container flex flex-wrap items-center justify-between gap-2">
						...
						<div class="flex items-center gap-2">
							<ButtonStyled v-if="ctx.openSettings" type="transparent" hover-color-fill="none">
								<button
					...
					{{
						ctx.modpack.value
							...
							: formatMessage(messages.emptyHint, {
									contentType: formatContentType(ctx.contentTypeLabel.value, 2),
								})
					}}
				</template>
				<template #actions>
