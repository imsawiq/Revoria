...
export interface ContentFilterConfig {
	...
	updateFilterLabel?: string
	clientOnlyFilterLabel?: string
	disabledFilterLabel?: string
}
	...
	const filterOptions = computed<ContentFilterOption[]>(() => {
		...
		if (config?.showTypeFilters) {
			...
			for (const type of types) {
				const label = config.formatProjectType ? config.formatProjectType(type) : type
		...
		if (
			...
			options.push({ id: 'updates', label: config?.updateFilterLabel ?? 'Updates' })
		}
		...
		if (
			...
			options.push({ id: 'client-only', label: config?.clientOnlyFilterLabel ?? 'Client-only' })
		}

		if (items.value.some((m) => !m.enabled)) {
			options.push({ id: 'disabled', label: config?.disabledFilterLabel ?? 'Disabled' })
		}
