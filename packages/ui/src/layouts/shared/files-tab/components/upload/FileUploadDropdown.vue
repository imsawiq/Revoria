...
const messages = defineMessages({
	...
	failedInferProjectType: {
		id: 'files.upload-dropdown.failed-infer-project-type',
		defaultMessage: 'Failed - Unable to infer project type for input file',
	},
})
...
const inferProjectTypeErrorMsg = 'Unable to infer project type for input file'

const localizeUploadErrorMessage = (message?: string) => {
	if (!message) {
		return formatMessage(messages.unexpectedError)
	}

	if (message.includes(inferProjectTypeErrorMsg)) {
		return formatMessage(messages.failedInferProjectType)
	}
		...
		if (index !== -1 && uploadQueue.value[index].status !== 'cancelled') {
			...
			if (error instanceof Error) {
				if (error.message === badFileTypeMsg) {
					...
					target.error = new Error(localizeUploadErrorMessage(error.message))
				}
			}
		}
