<template>
	<Transition name="collapsible-admonition">
		<div
			v-if="!dismissed"
			:data-type="type"
			class="collapsible-admonition flex flex-col overflow-hidden rounded-2xl border border-solid text-contrast"
		>
			<div
			...
			>
				...
				<div class="flex items-center gap-2">
					<ButtonStyled circular type="highlight-colored-text" :color="buttonColors[type]">
						<button :aria-label="formatMessage(messages.toggle)" @click.stop="expanded = !expanded">
							<ChevronDownIcon
					...
					>
						<button :aria-label="formatMessage(messages.dismiss)" @click.stop="handleDismiss">
							<XIcon class="h-4 w-4" />
						</button>
					</ButtonStyled>
				</div>
...
const messages = defineMessages({
	toggle: {
		id: 'console.admonition.toggle',
		defaultMessage: 'Toggle',
	},
	dismiss: {
		id: 'console.admonition.dismiss',
