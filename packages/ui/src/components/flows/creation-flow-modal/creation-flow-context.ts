...
import { defineMessages, type MessageDescriptor } from '@vintl/vintl'
...
const messages = defineMessages({
	titleWorld: {
		id: 'creation-flow.title.world',
		defaultMessage: 'Create world',
	},
	titleServerOnboarding: {
		id: 'creation-flow.title.server-onboarding',
	...
	titleInstance: {
		id: 'creation-flow.title.instance',
		defaultMessage: 'Create instance',
	},
	loaderVanilla: {
		id: 'creation-flow.loader.vanilla',
		defaultMessage: 'Vanilla',
	},
	defaultWorldName: {
		id: 'creation-flow.world.default-name',
		defaultMessage: 'World {count}',
	},
})
...
export function createCreationFlowContext(
	...
	const formatMessage =
		...
		((descriptor: MessageDescriptor, values?: Record<string, unknown>) => {
			if (descriptor.defaultMessage) {
				if (!values) return descriptor.defaultMessage
				return descriptor.defaultMessage.replace(/\{(\w+)\}/g, (_, key) =>
					String(values[key] ?? ''),
				)
			}
	...
	const autoInstanceName = computed(() => {
		...
		const loaderName = loader
			...
			: formatMessage(messages.loaderVanilla)
	...
	async function reset() {
		...
		worldName.value =
			flowType === 'world'
				? formatMessage(messages.defaultWorldName, { count: worldCounter })
				: ''
...
export function getFlowTypeHeading(
	ctx: CreationFlowContextValue,
	formatMessage: CreationFlowContextValue['formatMessage'],
): string {
	switch (ctx.flowType) {
		case 'world':
			return formatMessage(messages.titleWorld)
		case 'server-onboarding':
			return formatMessage(messages.titleServerOnboarding)
		case 'reset-server':
			return formatMessage(messages.titleResetServer)
		case 'instance':
		default:
			return formatMessage(messages.titleInstance)
	}
}
