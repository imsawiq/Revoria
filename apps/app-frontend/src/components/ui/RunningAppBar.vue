<template>
	<div class="action-groups">
		<ButtonStyled v-if="currentLoadingBars.length > 0" color="brand" type="transparent" circular>
			<button ref="infoButton" @click.stop="toggleCard()">
				<DownloadIcon />
			</button>
		</ButtonStyled>
...
const messages = defineMessages({
	offline: { id: 'running-app-bar.offline', defaultMessage: 'Offline' },
	stopInstance: { id: 'running-app-bar.stop-instance', defaultMessage: 'Stop instance' },
	viewLogs: { id: 'running-app-bar.view-logs', defaultMessage: 'View logs' },
	noInstancesRunning: {
...
const refreshInfo = async () => {
	...
	if (currentLoadingBars.value.length === 0) {
		...
		userClosedCard.value = false
		return
	}

	if (currentLoadingBarCount < currentLoadingBars.value.length) {
		showCard.value = true
		userClosedCard.value = false
		return
	}

	if (!userClosedCard.value) {
		showCard.value = true
	}
}
...
const handleClickOutsideCard = (event) => {
	...
	if (infoButton.value && path.includes(infoButton.value)) return
	if (card.value && card.value.$el && path.includes(card.value.$el)) return
	if (card.value && path.includes(card.value)) return
...
const toggleCard = async () => {
	...
	userClosedCard.value = !showCard.value
	showProfiles.value = false
	await refreshInfo()
}
