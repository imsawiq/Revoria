...
const props = withDefaults(
	defineProps<{
		...
		signIn: () => void
		refreshCredentials?: () => void | Promise<void>
		hideHeading?: boolean
	}>(),
	{
		hideHeading: false,
...
<template>
	...
	<div v-if="userCredentials && !loading" class="flex gap-1 items-center mb-3">
		...
		<h3 v-else-if="!props.hideHeading" class="w-full text-base text-primary font-medium m-0">
	...
	<div class="flex flex-col gap-3">
		<h3 v-if="loading && !props.hideHeading" class="text-base text-primary font-medium m-0">
			{{ formatMessage(messages.friends) }}
		</h3>
		...
		<template v-else-if="sortedFriends.length === 0">
			<div
				class="rounded-xl bg-[--color-glass-bg-strong] border border-[--glass-border] shadow-[--glass-shadow] p-4"
			>
				<div class="flex items-start gap-3">
					<div class="w-9 h-9 rounded-lg bg-button-bg flex items-center justify-center shrink-0">
						<UserIcon class="w-5 h-5 text-brand" />
					</div>
					<div class="text-sm leading-tight">
						...
						<div v-else>
							<IntlFormatted :message-id="messages.addFriendsToShare">
								<template #link="{ children }">
									<span
										class="font-semibold text-brand cursor-pointer"
										@click="addFriendModal.show"
									>
										<component :is="() => children" />
									</span>
