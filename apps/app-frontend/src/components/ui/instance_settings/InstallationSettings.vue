...
type AuthLibPatchingModal = {
	show: () => void
	hide: () => void
}

const authLibPatchingModal = ref<AuthLibPatchingModal | null>(null)
...
async function handleInitAuthLibPatching(ismojang: boolean) {
	...
	let instance_path =
		props.instance.loader_version != null
			? props.instance.game_version + '-' + props.instance.loader_version
...
<template>
			...
			<h2 class="text-lg font-bold text-contrast space-y-2">
				<p class="flex items-center gap-2 neon-text">
					...
					<span v-else class="neon-text">
						Failed to install AuthLib. It's possible that no compatible AuthLib version was found
						for the selected game and/or mod loader version. There may also be a problem with
						accessing resources behind CloudFlare.
					</span>
				</p>
			</h2>
		</div>
	...
	<div>
		...
		<template v-if="!instance.linked_data || !instance.linked_data.locked">
			...
			<h2 class="m-0 mt-4 text-lg font-extrabold text-contrast block">
				<div
					v-if="isAuthLibPatching"
					class="w-6 h-6 cursor-pointer hover:opacity-80 neon-icon pulse"
				>
