...
const formatLoaderLabel = (loader: string) => formatCategory(loader)
...
provideInstallationSettings({
	...
	installationInfo: computed(() => {
		const rows = [
			...
			{
				label: formatMessage(messages.gameVersionLabel),
	...
	resolveGameVersions(loader: string, showSnapshots: boolean) {
	...
	resolveLoaderVersions(loader: string, gameVersion: string) {
	...
	resolveHasSnapshots(loader: string) {
		const versions = all_game_versions?.value ?? []
	...
	async save(platform: string, gameVersion: string, loaderVersionId: string) {
	...
	async onModpackVersionConfirm(version: Labrinth.Versions.v2.Version) {
	...
	isServer: false,
	isApp: true,
	showModpackVersionActions: true,
	repairing,
	reinstalling,
})
</script>
