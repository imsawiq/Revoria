...
const messages = defineMessages({
	changeCape: { id: 'skins.action.change-cape', defaultMessage: 'Change cape' },
	noCape: { id: 'skins.cape.none', defaultMessage: 'No Cape' },
	none: { id: 'skins.none', defaultMessage: 'None' },
	...
	<ModalWrapper ref="modal" @on-hide="onModalHide">
		<template #title>
			<div class="flex flex-col">
				<span class="text-lg font-extrabold text-heading">{{ formatMessage(messages.changeCape) }}</span>
			</div>
		</template>
			...
			<div class="flex flex-col gap-4 w-full my-auto">
				<ScrollablePanel class="max-h-[20rem] max-w-[30rem] mb-5 h-full">
					<div class="flex flex-wrap gap-2 justify-center content-start overflow-y-auto h-full">
						<CapeLikeTextButton
							:tooltip="formatMessage(messages.noCape)"
						...
						>
							...
							<span>{{ formatMessage(messages.none) }}</span>
						</CapeLikeTextButton>
						<CapeButton
