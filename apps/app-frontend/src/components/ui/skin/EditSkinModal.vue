	...
	<ModalWrapper ref="modal" @on-hide="resetState">
		<template #title>
			<span class="text-lg font-extrabold text-contrast">
				{{ mode === 'edit' ? formatMessage(messages.editingSkin) : formatMessage(messages.addingSkin) }}
			</span>
		</template>
		...
		<div class="flex flex-col md:flex-row gap-6">
			...
			<div class="flex flex-col gap-4 w-full min-h-[20rem]">
				<section>
					<h2 class="text-base font-semibold mb-2">{{ formatMessage(messages.texture) }}</h2>
					<Button @click="openUploadSkinModal">
						<UploadIcon /> {{ formatMessage(messages.replaceTexture) }}
					</Button>
				</section>

				<section>
					<h2 class="text-base font-semibold mb-2">{{ formatMessage(messages.armStyle) }}</h2>
					<RadioButtons v-model="variant" :items="['CLASSIC', 'SLIM']">
						<template #default="{ item }">
							{{ item === 'CLASSIC' ? formatMessage(messages.wide) : formatMessage(messages.slim) }}
						</template>
					</RadioButtons>
				</section>

				<section>
					<h2 class="text-base font-semibold mb-2">{{ formatMessage(messages.cape) }}</h2>
					<div class="flex gap-2">
						...
						>
							<span>{{ formatMessage(messages.useDefaultCape) }}</span>
						</CapeButton>
						...
						<CapeLikeTextButton
							...
							:tooltip="formatMessage(messages.viewMoreCapes)"
		...
		<div class="flex gap-2 mt-12">
			<ButtonStyled color="brand" :disabled="disableSave || isSaving">
				<button v-tooltip="saveTooltip" :disabled="disableSave || isSaving" @click="save">
					...
					<SaveIcon v-else />
					{{ mode === 'new' ? formatMessage(messages.addSkin) : formatMessage(messages.saveSkin) }}
				</button>
			</ButtonStyled>
			<Button :disabled="isSaving" @click="hide"><XIcon />{{ formatMessage(messages.cancel) }}</Button>
		</div>
	</ModalWrapper>
...
const messages = defineMessages({
	editingSkin: { id: 'skins.editing', defaultMessage: 'Editing skin' },
	addingSkin: { id: 'skins.adding', defaultMessage: 'Adding a skin' },
	texture: { id: 'skins.texture', defaultMessage: 'Texture' },
	replaceTexture: { id: 'skins.replace-texture', defaultMessage: 'Replace texture' },
	armStyle: { id: 'skins.arm-style', defaultMessage: 'Arm style' },
	wide: { id: 'skins.arm-style.wide', defaultMessage: 'Wide' },
	slim: { id: 'skins.arm-style.slim', defaultMessage: 'Slim' },
	cape: { id: 'skins.cape', defaultMessage: 'Cape' },
	useDefaultCape: { id: 'skins.use-default-cape', defaultMessage: 'Use default cape' },
	viewMoreCapes: { id: 'skins.view-more-capes', defaultMessage: 'View more capes' },
	more: { id: 'project.more', defaultMessage: 'more' },
	addSkin: { id: 'skins.action.add-skin', defaultMessage: 'Add a skin' },
	saveSkin: { id: 'skins.action.save-skin', defaultMessage: 'Save skin' },
	cancel: { id: 'action.cancel', defaultMessage: 'Cancel' },
	saving: { id: 'skins.saving', defaultMessage: 'Saving...' },
	uploadSkinFirst: { id: 'skins.upload-first', defaultMessage: 'Upload a skin first!' },
	makeEditFirst: { id: 'skins.make-edit-first', defaultMessage: 'Make an edit to the skin first!' },
})
...
const saveTooltip = computed(() => {
	if (isSaving.value) return formatMessage(messages.saving)
	if (mode.value === 'new' && !uploadedTextureUrl.value) return formatMessage(messages.uploadSkinFirst)
	if (mode.value === 'edit' && !hasEdits.value) return formatMessage(messages.makeEditFirst)
	return undefined
})
