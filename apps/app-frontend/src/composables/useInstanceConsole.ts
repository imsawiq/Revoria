...
import { get_logs } from '@/helpers/logs'
...
function setHistoricalContent(profilePathId: string, filename: string, content: string): void {
	const entry = getOrCreate(profilePathId)
	entry.historicalCache.set(filename, content)
}
...
export function useInstanceConsole(profilePathId: string) {
	...
	return {
		...
		setHistoricalContent: (filename: string, content: string) =>
			setHistoricalContent(profilePathId, filename, content),
