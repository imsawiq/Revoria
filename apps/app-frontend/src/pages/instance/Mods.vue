...
const contentInstall = injectContentInstall(null)
const installingItems = contentInstall?.installingItems ?? ref(new Map())
...
async function bulkToggleMods(items: ContentItem[]) {
	for (const item of items) {
		await _toggleDisableMod(item)
	}
}
...
provideContentManager({
	...
	bulkEnableItems: (items) => bulkToggleMods(items),
	bulkDisableItems: (items) => bulkToggleMods(items),
